...
| CritiqueWorld deterministic parser | Yes | No | No | No | SMOKE_TEST_ONLY | `python -B -m user_simulator.evaluation.run_closed_loop_benchmark --modes none flat structured time_decay critiquescope --scenarios all --seeds 0 1 2 --max-turns 12 --top-k 5 --parser-mode deterministic --output-dir outputs/closed_loop_deterministic` | Uses a cue-based parser aligned to the built-in scenario utterances; parser errors are attributed separately. |
| CritiqueWorld validity gate | Yes | No | No | No | PASS | `python -B -m user_simulator.evaluation.run_validity_gate --modes none flat structured time_decay critiquescope --scenarios all --seeds 0 1 2 3 4 --max-turns 12 --top-k 5 --output-dir outputs/validity_gate --fail-on-critical-invariant` | CPU-only invariant audit; oracle parser reference run passes 100/100 invariants, and deterministic parser now passes 60/60 invariants on the built-in closed-loop scenarios. |
| GPE/HAP fixture bridge smoke | Yes | No | No | No | COMPLETED_FIXTURE_SMOKE | `bash scripts/run_fixture_rollout_bridge_smoke.sh` | Smoke-only fixture traces; real logs remain blocked when no trace files are present. |
| Server184 env + resource discovery | Yes | No | No | No | READY_FOR_GPU_SMOKE | `bash scripts/server184/check_env.sh && bash scripts/server184/discover_resources.sh` | Produces env/resource reports and blocker states without pretending a real rollout already ran. |
| Server184 real rollout smoke | Config only | Yes | Maybe | Yes | NOT_RUN | `bash scripts/server184/run_real_gimo_rollout_smoke.sh` | Requires server 184 access, local model weights, GPE/HAP task data, retrieval index, and API config. |
| Server184 full bridge pipeline | Config only | Maybe | Maybe | Yes | NOT_RUN | `bash scripts/server184/run_gpu_smoke_pipeline.sh` | Stops at explicit blocker states; never falls back to fixture traces. |
| LLaMA-Factory CDPO dry-run | Yes | No | No | No | BLOCKED_MODEL_MISSING | `bash scripts/validate_llamafactory_cdpo_dataset.sh outputs\gpe_hap_fixture_smoke\cdpo_dataset_fixture_smoke` | Needs a configured model path to reach READY_FOR_GPU_SMOKE. |
