...
NUM_EPOCHS=3.0
SFT_BETA=0
DATA_ROLE="search,recommend" # 可选: search, ask, recommend, all (多选用逗号分隔)
ADPO_KL_WEIGHT=1.0
# Define the output directory dynamically based on learning rate and num_epochs
OUTPUT_DIR="/home/u2023000157/ecpo/fine-tuning/sto_adpo_v1_sample1_${DATA_ROLE}/Yelp/lr${LEARNING_RATE}_epochs${NUM_EPOCHS}_sft${SFT_BETA}_adpo_kl${ADPO_KL_WEIGHT}"
...
llamafactory-cli train \
  ...
  --adapter_name_or_path /home/u2023000157/ecpo/fine-tuning/sft/Yelp/lr5.0e-5_epochs3.0\
  --ref_model /home/share/pretrain_models/Meta-Llama-3.1-8B-Instruct \
  --ref_model_adapters /home/u2023000157/ecpo/fine-tuning/sft/Yelp/lr5.0e-5_epochs3.0 \
  --stage dpo \
  --do_train true \
  --pref_loss adpo \
  --adpo_kl_type abs \
  ...
  --lora_target all \
  --dataset yelp_sto_dpo_v1_sample1 \
  --template llama3 \
