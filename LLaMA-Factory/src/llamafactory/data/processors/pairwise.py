...
#     # 解析train_data_role
#     role_set = set()
#     train_data_role = (data_args.train_data_role or "all").strip().lower()
#     if train_data_role != "all":
#         role_set = set([r.strip().lower() for r in train_data_role.split(",")])

#     model_inputs = defaultdict(list)

#     for i in range(len(examples["_prompt"])):
#         # 取response[0]内容
#         response_content = examples["_response"][i][0]["content"].strip().lower()

#         # 根据开头判断类型
#         if response_content.startswith("ask["):
#             response_type = "ask"
#         elif response_content.startswith("recommend["):
#             response_type = "recommend"
#         elif response_content.startswith("search["):
#             response_type = "search"
#         else:
#             response_type = "other"

#         # Debug打印
#         logger.info_rank0(f"[DEBUG] Example {i}: response_type = {response_type}")

#         # 筛选
#         if role_set:
#             if response_type not in role_set:
#                 logger.info_rank0(f"[DEBUG] Skipped due to type: {response_type}")
#                 continue
...
def preprocess_pairwise_dataset(
    ...
    model_inputs = defaultdict(list)
    for i in range(len(examples["_prompt"])):
        # print(examples["_prompt"][i])
        # print(len(examples["_response"][i]))
        # print("===================")
        # print(examples["_response"][i])
        # print(len(examples["_response"][i]))
        # raise
        if len(examples["_prompt"][i]) % 2 != 1 or len(examples["_response"][i]) < 2:
            logger.warning_rank0(
